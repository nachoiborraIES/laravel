...
Y también podemos utilizar una función llamada `compact` como segundo parámetro de `view`. A esta función le pasamos únicamente el nombre de la variable y, siempre que la variable asociada se llame igual, establece la asociación por nosotros:
...
Si simplemente vamos a devolver una vista con poca información asociada, o poca lógica interna, también podemos abreviar el código anterior llamando directamente a `view`, en lugar de a `route` primero, en el archivo `routes/web.php`, y le pasamos así la información asociada a la vista:
...
En este tipo de iteradores (`@foreach` o `@forelse`), tenemos disponible un objeto llamado `$loop`, con una serie de propiedades sobre el bucle que estamos iterando, como por ejemplo `index` (posición dentro del array por la que vamos), o `count` (total de elementos), o `first` y `last` (booleanos que determinan si es el primer o último elemento, respectivamente), entre otros. Podemos ver todas las propiedades disponibles en este objeto llamando a `var_dump`:
...
Existen otros tipos de estructuras iterativas y selectivas en Blade, como `@while`, `@for` o `@switch`, y algunos otros. Podéis consultar sobre su uso en la [documentación oficial de Blade](https://laravel.com/docs/blade#control-structures).
...
Además, mediante Blade existe una tercera forma de enlazar, empleando la función `url`, que genera una URL completa hasta la ruta que indiquemos:
...
#### 2.3.1. Incluir vistas dentro de otras
...
Para incluir el menú en la plantilla anterior, podemos hacer esto (y eliminaríamos el menú `<nav>` de la plantilla):
...
#### 2.3.2. Estructurar vistas en carpetas
...
### 2.4. Vistas para páginas de error
